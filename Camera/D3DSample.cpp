...
namespace camera
{
	...
	void D3DSample::OnResize()
	{
		...
		mCamera.SetLens(0.25f * MathHelper::Pi, GetAspectRatio(), 1.f, 1000.f);
	}

	void D3DSample::Update(float deltaTime)
	{
		if (GetAsyncKeyState('W') & 0x8000)
			mCamera.TranslateLook(10.0f * deltaTime);

		if (GetAsyncKeyState('S') & 0x8000)
			mCamera.TranslateLook(-10.0f * deltaTime);

		if (GetAsyncKeyState('A') & 0x8000)
			mCamera.TranslateRight(-10.0f * deltaTime);

		if (GetAsyncKeyState('D') & 0x8000)
			mCamera.TranslateRight(10.0f * deltaTime);

		if (GetAsyncKeyState('Q') & 0x8000)
			mCamera.TranslateUp(10.0f * deltaTime);
		...
		mSpotLight.Direction = DirectX::SimpleMath::Vector3(mCamera.GetLook());
	...
	void D3DSample::OnMouseMove(WPARAM btnState, int x, int y)
	{
		if ((btnState & MK_LBUTTON) != 0)
		{
			...
			mCamera.RotatePitch(dy);
			mCamera.RotateAxis({ 0,1,0 }, dx);
		}
	...
	void D3DSample::drawModel(const Model& model, const Matrix& worldMatrix)
	{
		...
		mCBPerObject.WorldViewProj = worldMatrix * view * proj;
